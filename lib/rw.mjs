...
import { TEMPLATE_DIRECTORY } from './constant.mjs';
...
export const run = async (sPath, tPath) => {
  filename = tPath;
  templatename = sPath.match(TEMPLATE_DIRECTORY) ? sPath : `${TEMPLATE_DIRECTORY}${sPath}`;
...
const rmdir = (tPath) => {
  Inquirer.prompt({
    ...
    .then(async (answers) => {
      if (['y', 'yes'].includes(answers.rmdir)) {
...
const nextGeneration = () => {
  Inquirer.prompt({
    ...
    .then(async (answers) => {
      if (['y', 'yes'].includes(answers.next)) {
